...
import { Edit, Trash2, Eye, EyeOff, ArrowUp, ArrowDown, Stethoscope } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Empty, EmptyHeader, EmptyMedia, EmptyTitle, EmptyDescription } from "@/components/ui/empty"
import {
  toggleDiseaseCaseActive,
  deleteDiseaseCase,
  moveDiseaseCase,
...
export function FeaturedDiseaseCasesTable({ cases }: { cases: DiseaseCase[] }) {
  ...
  function handleToggle(id: string, current: boolean) {
    ...
    startTransition(async () => {
      try {
        await toggleDiseaseCaseActive(id, !current)
      } finally {
        setBusyId(null)
      }
    })
  ...
  function handleMove(id: string, direction: "up" | "down") {
    ...
    startTransition(async () => {
      try {
        await moveDiseaseCase(id, direction)
      } finally {
        setBusyId(null)
      }
    })
                ...
                <td className="whitespace-nowrap px-4 py-3 text-right">
                  <div className="inline-flex items-center gap-1">
                    ...
                    <Button variant="ghost" size="icon" className="h-8 w-8" asChild>
                      <Link href={`/admin/featured-disease-cases/${c.id}/edit`} aria-label="编辑">
